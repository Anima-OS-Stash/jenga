...
// gcc -O3 -std=gnu99 $(pkg-config --cflags --libs gtkwidget2 libfile gstreamer-1.0 gstreamer-video-1.0) my-media-player.c
...
static unsigned long id_gtkwidget2; static int screen_width, screen_height, screen_span; static char **command; static char **place; static GstElement **player; static int n_pages, this_page, wanted_page; static GdkGeometry sizing; static char *to; static char stop;
...
static gboolean rule_player (GstBus *_player, GstMessage *_message, gpointer _parcel);
...
static gboolean when_clicked_button (GtkWidget *_w, GdkEvent *_e, void *_x) {
	...
	*/
	if (_w1->index == -1) gtk_widget_destroy (gtk_widget_get_toplevel (_w));
		...
		GstState _now; gst_element_get_state (player[this_page], &_now, NULL, GST_CLOCK_TIME_NONE);
		if (_now != GST_STATE_NULL) gst_element_set_state (player[this_page], GST_STATE_NULL);
		g_object_set (player[this_page], "uri", NULL, NULL);
		gst_video_overlay_set_window_handle ((GstVideoOverlay *) player[this_page], (guintptr) gtk_widget_get_window (session (this_page)));
		GtkWidget2 *_button = (GtkWidget2 *) chase (page[this_page], 2, 0, 3); tag3 *_tag = (tag3 *) _button->data;
		free (_tag->text1); _tag->text1 = strdup ("/etc/pictures/right.svg"); button3_refresh (_button);
		...
		char *_file2 = libtext_connect (2, "file://", _file1); if (_file2 == NULL) {free (_file1); return TRUE;}
		GstState _now; gst_element_get_state (player[this_page], &_now, NULL, GST_CLOCK_TIME_NONE);
		if (_now != GST_STATE_NULL) gst_element_set_state (player[this_page], GST_STATE_NULL);
		g_object_set (player[this_page], "uri", _file2, NULL);
		GtkWidget2 *_button = (GtkWidget2 *) chase (page[this_page], 2, 0, 3); tag3 *_tag = (tag3 *) _button->data;
		free (_tag->text1); _tag->text1 = strdup ("/etc/pictures/right.svg"); button3_refresh (_button);
		free (place[this_page]); place[this_page] = strdup (_file1); gtk_label_set_text ((GtkLabel *) pages_label, place[this_page]);
		free (_file2); free (_file1);
	}
	...
	else if (_w1->index == 17) {
		GstState _now; gst_element_get_state (player[this_page], &_now, NULL, GST_CLOCK_TIME_NONE);
		GtkWidget2 *_button = (GtkWidget2 *) chase (page[this_page], 2, 0, 3); tag3 *_tag = (tag3 *) _button->data;
		if (_now == GST_STATE_PLAYING) {
			GstStateChangeReturn _result = gst_element_set_state (player[this_page], GST_STATE_PAUSED);
			if ((_result == GST_STATE_CHANGE_SUCCESS) || (_result == GST_STATE_CHANGE_ASYNC)) {
				free (_tag->text1); _tag->text1 = strdup ("/etc/pictures/right.svg"); button3_refresh (_button);
			}
		}
		else {
			GstStateChangeReturn _result = gst_element_set_state (player[this_page], GST_STATE_PLAYING);
			if ((_result == GST_STATE_CHANGE_SUCCESS) || (_result == GST_STATE_CHANGE_ASYNC)) {
				free (_tag->text1); _tag->text1 = strdup ("/etc/pictures/pause.svg"); button3_refresh (_button);
			}
		}
	}
...
int main (int argc, char *argv[]) {
	...
	command = (char **) malloc (sizeof (char *) * 2); if (command == NULL) {printf ("[my-media-player] More memory, please! T_T\n"); exit (1);}
	...
	gtk_window_set_title ((GtkWindow *) overall, "my-media-player");
...
static void show_nth_page (int _nth, int _from) {
	...
	GList *_list = gtk_container_get_children ((GtkContainer *) page[_nth]); if (_list == NULL) {printf ("[my-media-player] More memory, please! T_T\n"); quit_program (NULL, NULL);}
...
static void *new_page (GtkWidget *widget, void *data) {
	...
	if (_page == NULL) {printf ("[my-media-player] More memory, please! T_T\n"); quit_program (NULL, NULL);}
	int _nth; for (_nth = 0; _nth < n_pages; _nth++) {
		_page[_nth] = page[_nth];
	}
	_page[_nth] = gtk_vbox_new (FALSE, 0); printf ("[my-media-player] New page: %d\n", _page[_nth]);
	if (page != NULL) free (page);
	page = _page;
	
	// Expand place list
	char **_place = (char **) malloc (sizeof (char *) * (n_pages + 1));
	if (_place == NULL) {printf ("[my-media-player] More memory, please! T_T\n"); free (_page); quit_program (NULL, NULL);}
	...
	// Expand player list
	GstElement **_player = (GstElement **) malloc (sizeof (GstElement *) * (n_pages + 1));
	if (_player == NULL) {
		printf ("[my-media-player] More memory, please! T_T\n");
		free (_page);
		int _nth; for (_nth = 0; _nth < n_pages + 1; _nth++) free (place[_nth]); free (place);
		quit_program (NULL, NULL);
	}
	for (_nth = 0; _nth < n_pages; _nth++) {
		_player[_nth] = player[_nth];
	}
	_player[_nth] = gst_element_factory_make ("playbin", "playbin");
	GstBus *_bus = gst_pipeline_get_bus ((GstPipeline *) _player[_nth]); gst_bus_add_watch (_bus, rule_player, _player[_nth]); gst_object_unref (_bus);
	...
	gtk_box_pack_start ((GtkBox *) _toolbar, button2 ('p', screen_span, screen_span, "/etc/pictures/right.svg", NULL, 17), FALSE, FALSE, 0);
	
	GtkWidget *_content = gtk_scrolled_window_new (NULL, NULL); gtk_box_pack_start ((GtkBox *) _page[_nth], _content, TRUE, TRUE, 0);
	gtk_scrolled_window_add_with_viewport ((GtkScrolledWindow *) _content, button3 (screen_width / 2, screen_height / 2, 0, NULL, NULL));
...
static void *remove_page (GtkWidget *widget, void *data) {
	GtkWidget *_this_page = page[this_page]; char *_this_place = place[this_page]; GstElement *_this_player = player[this_page];
	...
	_nth1 = 0; for (_nth = 0; _nth < n_pages; _nth++) {
		if (_nth == this_page) continue;
		_player[_nth1] = player[_nth]; _nth1++;
	}
	gst_element_set_state (_this_player, GST_STATE_NULL); gst_object_unref (_this_player);
	free (player);
	player = _player;
...
static void *show_page (GtkWidget *widget, void *data) {
	...
	GList *_list = gtk_container_get_children ((GtkContainer *) page[nth]); if (_list == NULL) {printf ("[my-media-player] More memory, please! T_T\n"); quit_program (NULL, NULL);}
...
static gboolean rule_player (GstBus *_bus, GstMessage *_message, gpointer _parcel) {
	if (_parcel == NULL) return TRUE;
	gint64 _length = 0; gst_element_query_duration ((GstElement *) _parcel, GST_FORMAT_TIME, &_length);
	if (_message->type == GST_MESSAGE_EOS) {
		gst_element_seek ((GstElement *) _parcel, 1.0, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_SET, _length - 1);
		gst_element_set_state ((GstElement *) _parcel, GST_STATE_PLAYING);
	}
	return TRUE;
}

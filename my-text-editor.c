...
// gcc -O3 -std=gnu99 $(pkg-config --cflags --libs gtkwidget2 libfile gtksourceview-2.0) my-text-editor.c
...
static gboolean when_clicked_button (GtkWidget *_w, GdkEvent *_e, void *_x) {
	...
	*/
	if (_w1->index == -1) gtk_widget_destroy (gtk_widget_get_toplevel (_w));
		...
		gtk_text_view_set_buffer ((GtkTextView *) session (this_page), gtk_text_buffer_new (NULL));
	}
		...
		char *_content = libfile_read (_file1, 0, -1); if (_content == NULL) {free (_file1); return TRUE;}
		GtkTextBuffer *_content1 = gtk_text_buffer_new (NULL); gtk_text_buffer_set_text (_content1, _content, -1);
		gtk_text_view_set_buffer ((GtkTextView *) session (this_page), _content1);
		free (place[this_page]); place[this_page] = strdup (_file1); gtk_label_set_text ((GtkLabel *) pages_label, place[this_page]);
		free (_content); free (_file1);
	}
		...
		if (_file == NULL) return TRUE;
		char *_file1 = libfile_refine_address (_file); free (_file); if (_file1 == NULL) return TRUE;
		int _file2 = open (_file1, O_RDWR | O_CREAT | O_TRUNC, 0600); if (_file2 == -1) {free (_file1); return TRUE;} close (_file2);
		GtkTextBuffer *_content = gtk_text_view_get_buffer ((GtkTextView *) session (this_page));
		GtkTextIter _from, _to; gtk_text_buffer_get_iter_at_offset (_content, &_from, 0); gtk_text_buffer_get_iter_at_offset (_content, &_to, -1);
		char *_content1 = gtk_text_buffer_get_text (_content, &_from, &_to, FALSE); if (_content1 == NULL) {free (_file1); return TRUE;}
		libfile_write (_file1, _content1, 's', 0, 'y', NULL);
		free (place[this_page]); place[this_page] = strdup (_file1); gtk_label_set_text ((GtkLabel *) pages_label, place[this_page]);
		free (_content1); free (_file1);
	}
...
int main (int argc, char *argv[]) {
	...
	command = (char **) malloc (sizeof (char *) * 2); if (command == NULL) {printf ("[my-text-editor] More memory, please! T_T\n"); exit (1);}
	...
	gtk_window_set_title ((GtkWindow *) overall, "my-text-editor");
...
static void show_nth_page (int _nth, int _from) {
	...
	GList *_list = gtk_container_get_children ((GtkContainer *) page[_nth]); if (_list == NULL) {printf ("[my-text-editor] More memory, please! T_T\n"); quit_program (NULL, NULL);}
...
static void *new_page (GtkWidget *widget, void *data) {
	...
	if (_page == NULL) {printf ("[my-text-editor] More memory, please! T_T\n"); quit_program (NULL, NULL);}
	int _nth; for (_nth = 0; _nth < n_pages; _nth++) {
		_page[_nth] = page[_nth];
	}
	_page[_nth] = gtk_vbox_new (FALSE, 0); printf ("[my-text-editor] New page: %d\n", _page[_nth]);
	if (page != NULL) free (page);
	page = _page;
	
	// Expand place list
	char **_place = (char **) malloc (sizeof (char *) * (n_pages + 1));
	if (_place == NULL) {printf ("[my-text-editor] More memory, please! T_T\n"); free (_page); quit_program (NULL, NULL);}
	...
	gtk_box_pack_start ((GtkBox *) _toolbar, button2 ('p', screen_span, screen_span, "/etc/pictures/floppy.svg", NULL, 15), FALSE, FALSE, 0);
	
	GtkWidget *_content = gtk_scrolled_window_new (NULL, NULL); gtk_box_pack_start ((GtkBox *) _page[_nth], _content, TRUE, TRUE, 0);
	GtkWidget *_content1 = gtk_source_view_new (); gtk_scrolled_window_add_with_viewport ((GtkScrolledWindow *) _content, _content1);
	gtk_source_view_set_auto_indent ((GtkSourceView *) _content1, TRUE);
	gtk_source_view_set_indent_on_tab ((GtkSourceView *) _content1, TRUE);
	gtk_source_view_set_indent_width ((GtkSourceView *) _content1, 4);
	gtk_source_view_set_highlight_current_line ((GtkSourceView *) _content1, TRUE);
	gtk_source_view_set_show_line_numbers ((GtkSourceView *) _content1, TRUE);
	gtk_source_view_set_tab_width ((GtkSourceView *) _content1, 4);
...
static void *show_page (GtkWidget *widget, void *data) {
	...
	GList *_list = gtk_container_get_children ((GtkContainer *) page[nth]); if (_list == NULL) {printf ("[my-text-editor] More memory, please! T_T\n"); quit_program (NULL, NULL);}

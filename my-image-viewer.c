...
// gcc -O3 -std=gnu99 $(pkg-config --cflags --libs gtkwidget2 libfile) my-image-viewer.c
...
static gboolean when_clicked_button (GtkWidget *_w, GdkEvent *_e, void *_x) {
	...
	*/
	if (_w1->index == -1) gtk_widget_destroy (gtk_widget_get_toplevel (_w));
		...
		GtkWidget *_box = chase (session (this_page), 2, -1, -1);
		gtk_widget_destroy (chase (session (this_page), 1, -1));
		gtk_scrolled_window_add_with_viewport ((GtkScrolledWindow *) _box, button3 (screen_width / 2, screen_height / 2, 0, NULL, NULL)); gtk_widget_show_all (_box);
		...
		GdkPixbuf *_picture = picture2 (_file1, -1, -1); if (_picture == NULL) {free (_file1); return TRUE;}
		GtkWidget *_box = chase (session (this_page), 2, -1, -1);
		gtk_widget_destroy (chase (session (this_page), 1, -1));
		gtk_scrolled_window_add_with_viewport ((GtkScrolledWindow *) _box, button3 (gdk_pixbuf_get_width (_picture), gdk_pixbuf_get_height (_picture), 1, _file1, NULL)); gtk_widget_show_all (_box);
		free (place[this_page]); place[this_page] = strdup (_file1); gtk_label_set_text ((GtkLabel *) pages_label, place[this_page]);
		g_object_unref (_picture); free (_file1);
	}
...
int main (int argc, char *argv[]) {
	...
	command = (char **) malloc (sizeof (char *) * 2); if (command == NULL) {printf ("[my-image-viewer] More memory, please! T_T\n"); exit (1);}
	...
	gtk_window_set_title ((GtkWindow *) overall, "my-image-viewer");
...
static void show_nth_page (int _nth, int _from) {
	...
	GList *_list = gtk_container_get_children ((GtkContainer *) page[_nth]); if (_list == NULL) {printf ("[my-image-viewer] More memory, please! T_T\n"); quit_program (NULL, NULL);}
...
static void *new_page (GtkWidget *widget, void *data) {
	...
	if (_page == NULL) {printf ("[my-image-viewer] More memory, please! T_T\n"); quit_program (NULL, NULL);}
	int _nth; for (_nth = 0; _nth < n_pages; _nth++) {
		_page[_nth] = page[_nth];
	}
	_page[_nth] = gtk_vbox_new (FALSE, 0); printf ("[my-image-viewer] New page: %d\n", _page[_nth]);
	if (page != NULL) free (page);
	page = _page;
	
	// Expand place list
	char **_place = (char **) malloc (sizeof (char *) * (n_pages + 1));
	if (_place == NULL) {printf ("[my-image-viewer] More memory, please! T_T\n"); free (_page); quit_program (NULL, NULL);}
...
static void *show_page (GtkWidget *widget, void *data) {
	...
	GList *_list = gtk_container_get_children ((GtkContainer *) page[nth]); if (_list == NULL) {printf ("[my-image-viewer] More memory, please! T_T\n"); quit_program (NULL, NULL);}
